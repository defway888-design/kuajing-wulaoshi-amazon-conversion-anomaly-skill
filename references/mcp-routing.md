...
| 两期父子商品`sessions_total`与`volume` | 优先使用上游同批次已确认结果；缺失时使用当前环境中已确认支持父/子ASIN与日期区间汇总的领星 MCP能力 |
...
运行时通过当前工具目录或工具搜索发现领星、西柚和卖家精灵 MCP。先按本文件规定的逻辑工具名核对能力，再为整次任务分别固定一个领星命名空间、一个西柚命名空间和一个卖家精灵命名空间；不得因为字段缺失切换命名空间补数。

AI工具环境适配规则：

- 本文件中的领星、西柚和卖家精灵名称表示逻辑数据源，不代表执行人员环境中必须存在同名服务器或固定工具前缀。
- 优先使用原生 MCP。若当前 AI 或 Agent 框架通过函数、连接器或工具适配层暴露能力，只有在底层仍访问同一个领星、西柚或卖家精灵数据源，并保持本文件规定的字段和时间口径时，才可映射为对应逻辑工具；不得用其他供应商或其他算法结果替代。
- 只使用当前任务实际暴露的工具目录、工具搜索、函数目录或连接器资源发现工具；不得读取本地 AI客户端配置文件、Agent框架配置文件、MCP配置文件、服务器URL、请求头、Token或密钥来定位工具。
- 不要求执行人员把 MCP安装在固定目录，也不自行安装、启用、修改或重启 MCP服务。
- 同一逻辑工具在不同环境中具有不同命名空间时，以当前工具目录实际返回的完整名称调用；缓存键记录本次实际选定的命名空间。
- 必需逻辑工具未发现、无权限或契约不满足时，按数据不足处理并说明缺失的逻辑工具，不得搜索作者电脑目录或其他本地 Skill兜底。
...
3. 上游同批次结果或领星：父商品及全部子体两期`sessions_total`与`volume`
4. 业务层：按 `parent-child-contribution.md` 拆解贡献并选择累计绝对贡献80%、最多5个重点子体
5. 西柚取得重点子体当前标题；卖家精灵取得重点子体当前主图、五点、最细叶子类目和小类 BSR；高优先级目标父 ASIN五点为空时按 4.1.1 补抓子 ASIN五点
6. 卖家精灵：为每个重点子体建立叶子类目 BSR 1—100候选
7. 西柚批量取得候选当前标题；卖家精灵取得候选主图、五点、父子关系和预测近30天销量
8. 若不足五个：放弃对应重点子体的 BSR池，用主要自然流量词 Top 10重建候选池
9. 西柚：重点子体和最终竞品标题修改历史
10. 西柚：重点子体和最终竞品 Prime价格日趋势
11. 卖家精灵：重点子体和最终竞品 Keepa聚合趋势
12. 卖家精灵：重点子体和最终竞品 Coupon
13. 卖家精灵：重点子体负面评论和竞品全部评论
14. 业务层：时间切分、证据分组、方向判断、结构化证据和运营可读报告
```

每个重点子体的竞品集合只生成一次并在该子体本次任务中复用。标题修改和 Prime价格只查询重点子 ASIN及其最终竞品代表子 ASIN。不得在贡献筛选前对全部子体运行高成本外部诊断。

## 3. 领星：当前自有商品身份

### 3.1 店铺、站点和 Seller ID

逻辑工具：领星 `get_my_sids`。运行时使用当前工具目录中已选定领星命名空间对应的完整工具名，不固定为某个服务器前缀。
...
逻辑工具：领星 `erp_listing`。运行时使用当前工具目录中已选定领星命名空间对应的完整工具名，不固定为某个服务器前缀。
...
### 3.3 两期父子商品sessions_total与volume

优先复用上游周数据分析主 Skill在同一记录键、同一批次、同一比较窗口内已经确认的父商品及子体汇总结果，避免重复调用领星。上游结果必须包含站点、店铺、父 ASIN、子 ASIN、实际比较窗口、`sessions_total`和`volume`，并通过父子汇总一致性校验。

上游未提供子体数据时，只能在当前工具目录中选择已确认支持以下语义能力的领星工具：

```text
按站点和店铺限定
按父ASIN或子ASIN限定
按开始日期和结束日期限定
返回子ASIN、sessions_total和volume
支持完整分页或区间汇总
```

不同环境的工具名和字段名可以变化，按语义映射读取 `父ASIN / 子ASIN / sessions_total / volume / 日期窗口`；只有确认语义等价时才允许映射，不得把销量预测、广告订单、广告成交量或其他来源替代店铺经营口径的`volume`。工具契约无法确认时，不猜测工具名或字段。

固定计算口径：

```text
期间子体转化率 = 期间子体volume合计 / 期间子体sessions_total合计
期间父商品转化率 = 全部子体期间volume合计 / 全部子体期间sessions_total合计
```

取得数据后执行 `parent-child-contribution.md`。如果只能取得父商品汇总而无法取得子体明细，父商品核心转化率仍可用于父商品层诊断，但证据缺口必须写 `子体定位不足`；不得调用其他 MCP补子体店铺经营数据。
...
用于重点子 ASIN、BSR候选和关键词候选的当前标题；独立运行时重点子 ASIN为用户指定异常子 ASIN。多个 ASIN优先合并到同一次列表调用；超过工具限制时按原候选顺序分批，禁止按另一业务指标重排。
...
每个重点子 ASIN和每个最终竞品代表子 ASIN各调用一次：
...
每个目标父商品优先选择绝对贡献最高的重点子 ASIN作为评论代表，基准期和异常期分别调用；子体定位不足时使用父商品层可验证的评论代表并记录缺口：
...
- 同一重点子 ASIN、同一竞品代表子 ASIN和同一日期窗口不得重复调用西柚相同工具。
- 多个重点子 ASIN复用同一最终竞品时，只有时间窗口相同或已有窗口为完整超集才复用历史结果。
